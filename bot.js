...
// ─── DM: /add (multi-step with options) ─────────────────────────────────────

const addSessions = {}; // userId → { step, question, options, answer }
...
bot.onText(/\/add(?:@.+)?(?:\s+(.+))?$/, (msg, match) => {
  ...
  if (inlineQuestion) {
    addSessions[msg.from.id] = { step: "options", question: inlineQuestion };
    return bot.sendMessage(
      msg.chat.id,
      `✅ Question saved!\n\n📝 <b>Step 2/3</b> — Send the <b>options</b>, one per line:\n\n<code>A. Option one\nB. Option two\nC. Option three\nD. Option four</code>`,
      { parse_mode: "HTML" }
    );
  }
  ...
  bot.sendMessage(
    msg.chat.id,
    "📝 <b>Step 1/3</b> — Type the <b>question</b>:\n\n(Send /cancel to abort)",
    { parse_mode: "HTML" }
  );
});
...
bot.on("message", (msg) => {
  ...
  if (session.step === "question") {
    ...
    if (lines.length < 2) {
      return bot.sendMessage(msg.chat.id, "⚠️ Please send at least 2 options, one per line.");
    }
    session.options = lines;
    session.step = "answer";
    bot.sendMessage(
      msg.chat.id,
      `✅ Options saved!\n\n📝 <b>Step 3/3</b> — Type the <b>correct answer</b> (e.g. <code>A</code> or the full text, case-insensitive):`,
      { parse_mode: "HTML" }
    );
    ...
    const optionsText = session.options.map(o => `  ${escapeHtml(o)}`).join("\n");
    bot.sendMessage(
      msg.chat.id,
      `🎉 <b>Question added!</b>\n\n` +
...
bot.onText(/\/drop/, async (msg) => {
  ...
  const optionsText = q.options && q.options.length
    ? "\n\n" + q.options.map(o => `▫️ ${escapeHtml(o)}`).join("\n")
    : "";
...
bot.on("message", async (msg) => {
  ...
  // Check direct match OR if user typed just the letter (e.g. "a" matches "a. injective")
  const optionMatch = active.options
    ? active.options.some(
        (o) =>
          o.toLowerCase() === correctAnswer ||
          o.toLowerCase().startsWith(correctAnswer + ".") ||
          o.toLowerCase().startsWith(correctAnswer + " ")
      )
    : false;
